...
bucket_name="JLR-ultimate-bucket"
lambda_func_name="JLR-s3-lambda-function"
role_name="JLR-s3-lambda-sns"
email_address="narendrak1771@gmail.com"

# Create IAM Role for the project
role_response=$(aws iam create-role --role-name JLR-s3-lambda-sns --assume-role-policy-document '{
  "Version": "2012-10-17",
  "Statement": [{
...
aws s3 cp ./view.html s3://"$bucket_name"/view.html

# Create a Zip file to upload Lambda Function
zip -r JLR-s3-lambda-function.zip ./JLR-s3-lambda-function
...
aws lambda create-function \
  ...
  --runtime "python3.8" \
  --handler "JLR-s3-lambda-function/JLR-s3-lambda-function.lambda_handler" \
  --memory-size 128 \
  --timeout 30 \
  --role "arn:aws:iam::$aws_account_id:role/$role_name" \
  --zip-file "fileb://./JLR-s3-lambda-function.zip"

# Add Permissions to S3 Bucket to invoke Lambda
aws lambda add-permission \
  --function-name "$lambda_func_name" \
  --statement-id "JLR-s3-lambda-sns" \
  --action "lambda:InvokeFunction" \
  --principal s3.amazonaws.com \
  --source-arn "arn:aws:s3:::$bucket_name"

# Create an S3 event trigger for the Lambda function
LambdaFunctionArn="arn:aws:lambda:us-east-1:$aws_account_id:function:JLR-s3-lambda-function"
...
topic_arn=$(aws sns create-topic --name JLR-s3-lambda-sns --output json | jq -r '.TopicArn')
...
aws sns publish \
  ...
  --message "Dear Associate site is under maintenance"
